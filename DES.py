...
#主体分类器
class DESforIDS(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):

    def __init__(self, base_classifier=HoeffdingTreeClassifier(), window_size=500, max_classifier=20):
...
class DESforIDSwd(DESforIDS):
    def __init__(self, base_classifier=HoeffdingTreeClassifier(), window_size=500, max_classifier=20):
        DESforIDS.__init__(self, base_classifier=base_classifier, window_size=window_size, max_classifier=max_classifier)
...
class DESforIDSwr(DESforIDS):
    def __init__(self, base_classifier=HoeffdingTreeClassifier(), window_size=500, max_classifier=20, K1=5, K3=5):
        DESforIDS.__init__(self, base_classifier=base_classifier, window_size=window_size,
                           max_classifier=max_classifier)
        self.K1 = K1
        self.K3 = K3
        ...
        for n in range(N):
            ...
            if self.i == self.window_size:
                if len(self.ensemble) <= 0:
                    ...
                    X_block_res, Y_block_res = list(self.X_block), list(self.Y_block)
                    min_num = min(min(class_size(self.Y_block)) - 1, self.K1)
                    if min_num > 0 :
                        X_block_res, Y_block_res = SMOTE(k_neighbors=min_num).fit_resample(X=X_block_res, y=Y_block_res)
                    X_block_res, Y_block_res = np.array(X_block_res), np.array(Y_block_res)
                    if len(self.ensemble) >= self.max_classifier:
                        self.ensemble.pop(0)
                    if len(self.ensemble) > 0:
                        for i in range(len(self.ensemble)):
    ...
    def predict_proba(self, X):
        ...
        for n in range(N):
            ...
            find_knn = Knn()
            find_knn.fit(self.X_block, self.Y_block)
            neighbors, neighbors_y = find_knn.get_neighbers(x=x, knn=self.K3)
